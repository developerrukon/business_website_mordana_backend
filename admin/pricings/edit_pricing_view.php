<?php
require_once "select_single_pricing.php";
require_once "edit_pricing.php";
require_once "../inc/header.php";

?>
<!-- header controler start-->
<div class="container-fluid page__heading-container">
    <div class="page__heading d-flex align-items-end">
        <div class="flex">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="#">Home</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Edit Pricing</li>
                </ol>
            </nav>
            <h1 class="m-0"><i class="fa fa-pen text-primary"></i>Edit Pricing</h1>
        </div>
    </div>
</div>
<div class="container-fluid page__container">
    <!-- header controler end-->

    <!-- card card-for start-->
    <div class="card card-form">
        <div class="row no-gutters">
            <div class="col-lg-4 card-body">
                <p><strong class="headings-color">Default Forms</strong></p>
                <p class="text-muted">Moderna supports all of Bootstrap's default form styling in addition to a handful of new input types and features. Please <a href="https://getbootstrap.com/docs/4.1/components/forms/" target="_blank">read the official documentation</a> for a full list of options from Bootstrap's core library.</p>
            </div>
            <div class="col-lg-8 card-form__body card-body">
                <form method="POST">

                    <div class="form-group">
                        <label>Title:</label>
                        <input type="text" name="title" class="form-control" value="<?= $pricing['title'] ?>" />
                        <?php
                        if (isset($error['title_err'])) {
                            echo $error['title_err'];
                        }
                        ?>
                    </div>
                    <div class="form-group">
                        <label>Pricing Number:</label>
                        <input type="text" name="pricing_number" class="form-control" value="<?= $pricing['pricing_number'] ?>" />
                        <?php
                        if (isset($error['pricing_number_err'])) {
                            echo $error['pricing_number_err'];
                        }
                        ?>
                    </div>
                    <div class="form-group">
                        <label>Pricing Heading:</label>
                        <input type="text" name="pricing_heading" class="form-control" value="<?= $pricing['pricing_heading'] ?>" />
                        <?php
                        if (isset($error['pricing_heading_err'])) {
                            echo $error['pricing_heading_err'];
                        }
                        ?>
                    </div>
                    <div class="form-group">
                        <label>Button Text:</label>
                        <input type="text" name="btn_text" class="form-control" value="<?= $pricing['btn_text'] ?>" />
                        <?php
                        if (isset($error['btn_text_err'])) {
                            echo $error['btn_text_err'];
                        }
                        ?>
                    </div>
                    <div class="form-group">
                        <label>Buton Url:</label>
                        <input type="text" name="btn_url" class="form-control" value="<?= $pricing['btn_url'] ?>" />
                        <?php
                        if (isset($error['btn_url_err'])) {
                            echo $error['btn_url_err'];
                        }
                        ?>
                    </div>
                    <div class="form-group">
                        <label>Description :</label>
                        <textarea name="description" class="form-control" rows="8"><?= $pricing['description'] ?></textarea>
                        <p>add icon for ex: bx bx-check <a href="https://blade-ui-kit.com/blade-icons/bx-check">helf</a></p>

                        <?php
                        if (isset($error['descrip_err'])) {
                            echo $error['descrip_err'];
                        }
                        ?>
                    </div>
                    <button type="submit" name="submit" class="btn btn-primary">Update</button>
                </form>
            </div>
        </div>
    </div>
    <!-- card card-for end-->


    <!-- footer controler start-->

</div>
<?php
require_once "../inc/footer.php";
?>
<!-- footer controler end-->